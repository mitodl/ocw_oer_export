...
#     ocw_topics_mapping = ocw_topic_to_subject['OER Subject'].to_dict()
...
def get_oer_subjects_for_ocw_course(ocw_topics_mapping, ocw_course_topics):
    ...
    oer_subjects_list = [ocw_topics_mapping.get(topic['name']).split('|') for topic in ocw_course_topics]
...
def main():
    ...
    ocw_topics_mapping = create_ocw_topic_to_oer_subject_mapping()
    get_oer_subjects_for_ocw_course(ocw_topics_mapping,[{'id': 124, 'name': 'Environmental Engineering'}, {'id': 22, 'name': 'Engineering'}, {'id': 199, 'name': 'Civil Engineering'}])
